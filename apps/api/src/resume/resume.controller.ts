...
export class ResumeController {
  ...
  async generatePdf(@Body() body: any, @Res() res: express.Response) {
    ...
    const script = existsSync(scriptPath) ? scriptPath : resolve(process.cwd(), 'resume_pdf.py');
    ...
    pyProcess.on('close', (code) => {
      if (code !== 0) {
        ...
        res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({ error: 'PDF generation failed' });
        return;
      }
