...
export class MailService {
  ...
  async sendEmail(to: string, subject: string, htmlContent: string): Promise<boolean> {
    ...
    if (this.resendApiKey) {
      try {
        ...
        this.logger.log(`Email successfully dispatched to ${to} via Resend`);
        return true;
      } catch (error) {
        this.logger.error(`Resend failed for ${to}: ${(error as Error).message}`);
  ...
  private async sendViaSmtpFallback(to: string, subject: string, htmlContent: string): Promise<boolean> {
    const transporter = this.getTransporter();
    if (!transporter) {
      this.logger.warn('No email service configured (RESEND_API_KEY, SMTP_HOST, SMTP_USER, SMTP_PASS). Skipping email send.');
    ...
    try {
      ...
      this.logger.error(`SMTP email failed for ${to}: ${(error as Error).message}`);
      this.logger.log(`[MOCK EMAIL to ${to}] Subject: ${subject}`);
      return true;
    }
  }
