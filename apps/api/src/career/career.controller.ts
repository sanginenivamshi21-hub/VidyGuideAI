...
7. **Final Action Plan** — A "Your Next 30 Days" plan with 3 concrete next steps. ${langInstr}`;
    ...
    private classify(text: string): string {
        ...
        if (
            ...
            return 'job';
        if (
            ['build', 'project', 'portfolio', 'create', 'develop', 'make'].some(
                (w) => t.includes(w),
            )
        )
            return 'build';
        if (
            ['salary', '₹', 'lpa', 'income', 'earn', 'pay', 'money'].some((w) =>
                t.includes(w),
            )
        )
            return 'money';
        if (
            ['exam', 'test', 'gate', 'upsc', 'ssc', 'neet', 'jee', 'ibps'].some(
                (w) => t.includes(w),
            )
        )
            return 'exam';
        if (
            ['intern', 'apprentice', 'trainee', 'experience'].some((w) =>
                t.includes(w),
            )
        )
            return 'intern';
