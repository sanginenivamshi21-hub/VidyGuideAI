...
if [ -d ".venv" ]; then
  ...
  .venv/bin/pip install -r python/requirements.txt >> apps/api/dist/build-info.txt 2>&1
  ...
  pip3 install -r python/requirements.txt --user >> apps/api/dist/build-info.txt 2>&1
fi
